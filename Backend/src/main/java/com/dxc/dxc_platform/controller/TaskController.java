...
public class TaskController {
    ...
    @PostMapping @PreAuthorize("hasRole('CHEF_PROJET')")
    public ResponseEntity<TaskDto> createTask(@Valid @RequestBody TaskDto dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(taskService.createTask(dto)); }
    @PutMapping("/{id}") @PreAuthorize("hasRole('CHEF_PROJET')")
    public ResponseEntity<TaskDto> updateTask(@PathVariable Long id, @Valid @RequestBody TaskDto dto) {
        return ResponseEntity.ok(taskService.updateTask(id, dto)); }
    @GetMapping("/project/{projectId}") @PreAuthorize("hasRole('CHEF_PROJET')")
    public ResponseEntity<List<TaskDto>> getTasksByProject(@PathVariable Long projectId) {
        return ResponseEntity.ok(taskService.getTasksByProject(projectId)); }
    @GetMapping("/my-tasks") @PreAuthorize("isAuthenticated()")
    public ResponseEntity<List<TaskDto>> getMyTasks() {
        return ResponseEntity.ok(taskService.getMyTasks()); }
    @PatchMapping("/{id}/status") @PreAuthorize("isAuthenticated()")
    ...
    @DeleteMapping("/{id}") @PreAuthorize("hasRole('CHEF_PROJET')")
